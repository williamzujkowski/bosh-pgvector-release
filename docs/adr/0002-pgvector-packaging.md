...
package so the BOSH director compiles them in order. The packaging
script invokes `make PG_CONFIG=/var/vcap/packages/postgres-N/bin/pg_config`
which writes the `.so`, control file, and SQL into that postgres-N
...
- `OPTFLAGS=""` overrides pgvector's default `-march=native`. The BOSH
  ...
  for portability.
- `make install PG_CONFIG=...` — pgvector's Makefile uses PGXS, so
  PG_CONFIG fully determines where install goes.
